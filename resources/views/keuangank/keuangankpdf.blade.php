...
<head>
    ...
    <title>Laporan Pengeluaran Kantor</title>
    <style>
        @page {
			size: 8.3in 11.7in;
		}
...
<body>
    ...
    <div style="text-align: left; font-size: 12px;">
		<h3>Laporan Pengeluaran Dana SPPD Kantor Kategori {{ $search ? $search : 'Semua' }}</h3>
 ...
 <div style="text-align: left; font-size: 12px; margin-top: 20px;">
	<hr>
	<strong>Total Pengeluaran Kantor Kategori {{ $search ? $search : 'Semua' }}</strong>
	@php
		$totalPengeluaranKantor = 0;
		foreach ($items as $item) {
			$totalPengeluaranKantor += $item->subjumlah;
		}
	@endphp
	: Rp. {{ number_format($totalPengeluaranKantor, 0, ',', '.') }}
</div>
</body>
</html>
